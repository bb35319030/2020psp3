...
80:pNodeというリストのcityの領域にnewCity（リストに挿入する市町村のデータ）を代入する

81:pNodeというリストのpNextの領域にpNext（リストの先頭の値アドレス）を代入する

83:pNode(リストの先頭アドレス)をかえす

107:変数を用意する

108:pList(リストの先頭アドレス)をpNodeに代入する

109:pos=0を定義

110:アドレスがなくなるまでくり返す

111-114:IDとpNodeのcityの領域にあるidが合致した場合、 *pCityにIDが合致した市町村のデータを入れる

115-116:pNodeが次の要素をさすようにする。posにpos+1を代入する。

119:IDとpNodeのcityの領域にあるidが合致しなかった場合、-1をかえす
